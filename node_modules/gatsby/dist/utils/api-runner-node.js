...
const _require = require(`./stack-trace-utils`),
      getNonGatsbyCodeFrame = _require.getNonGatsbyCodeFrame; // Bind action creators per plugin so we can auto-add
...
const runAPI = (plugin, api, args) => {
  ...
  if (gatsbyNode[api]) {
    ...
    const _require3 = require(`../db/nodes`),
          loadNodeContent = _require3.loadNodeContent,
          getNodes = _require3.getNodes,
          getNode = _require3.getNode,
          getNodesByType = _require3.getNodesByType,
          hasNodeChanged = _require3.hasNodeChanged,
          getNodeAndSavePathDependency = _require3.getNodeAndSavePathDependency;
      ...
      doubleBoundActionCreators.createPage = (...args) => {
        ...
        if (apiFinished && !alreadyDisplayed) {
          const warning = [reporter.stripIndent(`
              ...
              For more info and debugging tips: see ${chalk.bold(`https://gatsby.app/sync-actions`)}
            `)];
